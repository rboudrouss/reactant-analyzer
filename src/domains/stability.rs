...
mod tests {
    ...
    fn join_stable_unstable_is_unknown() {
        assert_eq!(Stability::Stable.join(&Stability::Unstable), Stability::Unknown);
        assert_eq!(Stability::Unstable.join(&Stability::Stable), Stability::Unknown);
    }

    #[test]
    fn join_with_bottom_is_identity() {
        assert_eq!(Stability::Stable.join(&Stability::Bottom), Stability::Stable);
        assert_eq!(Stability::Unstable.join(&Stability::Bottom), Stability::Unstable);
        assert_eq!(Stability::Bottom.join(&Stability::Stable), Stability::Stable);
    }

    #[test]
    fn join_with_unknown_is_unknown() {
        assert_eq!(Stability::Stable.join(&Stability::Unknown), Stability::Unknown);
        assert_eq!(Stability::Unstable.join(&Stability::Unknown), Stability::Unknown);
        assert_eq!(Stability::Bottom.join(&Stability::Unknown), Stability::Unknown);
    }

    #[test]
    fn join_idempotent() {
        for v in [Stability::Bottom, Stability::Stable, Stability::Unstable, Stability::Unknown] {
    ...
    fn meet_stable_unstable_is_bottom() {
        assert_eq!(Stability::Stable.meet(&Stability::Unstable), Stability::Bottom);
    }

    #[test]
    fn meet_with_unknown_is_identity() {
        assert_eq!(Stability::Stable.meet(&Stability::Unknown), Stability::Stable);
        assert_eq!(Stability::Unstable.meet(&Stability::Unknown), Stability::Unstable);
    }
    ...
    fn literal_is_stable() {
        assert_eq!(Stability::from_expr_static(&Expr::Lit(Prim::Int(0))), Stability::Stable);
        assert_eq!(Stability::from_expr_static(&Expr::Lit(Prim::Bool(true))), Stability::Stable);
        assert_eq!(Stability::from_expr_static(&Expr::Lit(Prim::Null)), Stability::Stable);
    }

    #[test]
    fn object_lit_is_unstable() {
        assert_eq!(Stability::from_expr_static(&Expr::ObjectLit(vec![])), Stability::Unstable);
    }

    #[test]
    fn array_lit_is_unstable() {
        assert_eq!(Stability::from_expr_static(&Expr::ArrayLit(vec![])), Stability::Unstable);
    }
    ...
    fn fn_lit_is_unstable() {
        ...
        let cfg = CFG { entry: 0, blocks, edges: vec![] };
        assert_eq!(
            Stability::from_expr_static(&Expr::FnLit { params: vec![], body_cfg: Box::new(cfg) }),
            Stability::Unstable
        );
    }
    ...
    fn state_setter_is_stable() {
        assert_eq!(Stability::from_expr_static(&Expr::StateSetter(0)), Stability::Stable);
        assert_eq!(Stability::from_expr_static(&Expr::StateSetter(99)), Stability::Stable);
    }
