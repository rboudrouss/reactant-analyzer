...
//! values and slots, CFG/expression scans, the deps stability gate, and the
//! churn/setter submodules. Nothing here emits a diagnostic — that is the
//! rules' job ([`crate::rules::impls`]) through the typed surface
...
/// Moved to the engine (ADR-042 §6): the post-fixpoint evaluator is what every
