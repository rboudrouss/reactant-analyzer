...
| `elements` | Every **element** the render body builds, of the requested kinds (#126) — the anchor `jsx_props` could not be, because a rule about a prop's *absence* (`<input value={v}/>` with no `onChange`) needs the element as the subject and its props as an edge. Same optional `elements` filter and same default (`component`). `name` is the component name or the host tag, `kind` says which. One edge, `props` |
...
| `props` | `elements` | The props of the anchored element — the same rows `jsx_props` enumerates, grouped under the element that carries them, so `none of anchor.props` can ask whether one is missing |
