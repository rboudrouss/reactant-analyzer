...
| `jsx_props` | Every prop of every **resolved component element** the render body builds — including inside a callback it runs synchronously, so `items.map(it => <Row/>)` is enumerated (#125). Host elements (`<div/>`) produce no rows, and neither does an element built in an event handler. Inside a list callback there is no analysed env, so `identity` answers `fresh-every-render` for an allocation written at the site and `unknown` otherwise. `name` is the element, `prop` the prop's name, `identity` its value's identity verdict — the same verdict the provider relation uses. Which children memoize is unknown, so name them with a `name` guard. No `kind`, no edges |
