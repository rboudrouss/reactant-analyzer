...
> The entries here are over-approximations on the *diagnostic* side. The first
> one is different in kind: it is an over-suppression on the **assurance** side
> (`verified: …`), which costs no soundness and no diagnostic, only precision in
> what reactant is willing to certify.

- **The assurance channel is suspended per component, not per (limit kind, check)** — any `analysis-limit` withholds all 15 `SafeCheck`s for that component (`RuleRegistry::check_component`). Right for `unknown-hook` and `callback-depth-cap`, whose opaque region is *this component's own body* and can therefore hold a conditional `useState`, an undeclared capture or a diverging effect. Over-broad for `unknown-component` and `recursion-cutoff`, which are about a **child**: an unanalysed child cannot make its parent's hooks conditional. Verified — a component rendering an unknown child loses `conditional-hook` ("all hooks run unconditionally, in a stable order") and `lazy-init`, both properties of its own lexical hook sequence and its own initializers.

  **Prerequisite, not optional**: the limit kind is not a value. All four cases are emitted as the bare name `"analysis-limit"` and differ only in message prose, even though four doc comments in `src/` already write `analysis-limit/unknown-hook` as if it were a name. Step 1 is to split into four diagnostic names, which the existing machinery already supports (`RULE_DOCS` is keyed by diagnostic name, one rule may emit several — `InfiniteLoop` does; keep `--ignore-rule analysis-limit` working with a prefix match in `visible()`). Step 2 is then cheap and does **not** need a 15 × 4 matrix: the four kinds fall into "opaque region is my own body" (withhold everything) vs "about a child" (withhold only what crosses the boundary), and the 15 checks into local (`conditional-hook`, `lazy-init`, `derived-state`, `redundant-set-state`, `unnecessary-rerender`, `missing-cleanup`, `server-component-hook`) vs boundary-crossing (`setter-in-render` via `cross-setter-in-render`, `infinite-loop` via its cross-component arm, `frozen-initial-state` which resolves against a parent, `state-mutation` on props, `always-unstable-deps` and `missing-deps` on prop-rooted deps, `unstable-context-value`). That is 15 one-bit declarations, defaulting fail-closed so a forgotten bit withholds as today.

  Low priority: it only changes `--info` output, never a diagnostic or an exit code. The user-visible half of the problem — a truncated component being indistinguishable from one with nothing to check — is fixed as of 2026-08-27 by the `suspended` line (`ComponentFindings::suspended_safe_checks`, rendered by `driver::human::render_safe_checks`, documented in [usage.md](usage.md#the-assurance-channel---info)); the line is deliberately not subject to `--ignore-rule`, since silencing the notice must not silence the loss of the guarantee.
