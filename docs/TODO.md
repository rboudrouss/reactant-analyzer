...
- **Analyse inter-composants** résolution d'imports hors scope, composants dynamiques (`const C = cond ? A : B`) non tracés, plugin système (Next.js/TanStack) futur. *(Info émis via `analysis-limit` pour composants inconnus et récursifs — `--info` pour voir)*
...
- **`setter-in-render` / `cross-setter-in-render` FN sur callées inconnus** — détecte jusqu'à 2 niveaux de wrappers locaux (`doReset → setX`, `outer → inner → setX`) via B6 (depth cap = 3). Helpers importés depuis un autre module (sans `Loc` dans l'env) non descendent. *(Info émis via `analysis-limit` quand depth cap atteint — `--info` pour voir)*
