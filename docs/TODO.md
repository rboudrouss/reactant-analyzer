...
- **for-of / for-in loop head is never lowered** — `lower_iter_loop` walks only the body: the loop *variable* is never bound and the iterated *expression* never read. A loop var shadowing an outer binding reads as a capture of that binding → `missing-deps` FP when the outer name is non-stable (memos `useResolvedRelationMemos` inlined into `MemoDetailSidebar`: `for (const memo of …)` in a useMemo and in a setter-updater resolves to the component's `memo` prop → 3 FPs). Dual FN: the head read (`for (const x of foo)`) doesn't register, so omitting `foo` from deps goes unflagged. Fix pattern = the catch-param fix (dcb4d0e): `lower_binding_pattern(f.left, ⊤-or-element, …)` + lower `f.right`, in the loop header.
