...
- **⚠ IMPORTANT — `is_stable`/`is_unstable` non-complementarity drops `⊤`/`Versioned` → `infinite-loop` FN** — `is_stable()` (true only for `Stability::Stable`) and `is_unstable()` (true only for `PerRender`) are **not complements** (`state_value.rs:365-373`): a `Unknown` (⊤) or `Versioned` value falls in the gap and **both return `false`**. `all_deps_unstable` (`mod.rs:768`) is built on `.is_unstable()`, and `infinite_loop.rs:98` gates with `if !all_deps_unstable(deps) { continue }` — so an effect whose dep evaluates to ⊤ (precision lost; `to_stability` itself documents ⊤ as "may change every render", `state_value.rs:285`) yields `is_unstable() == false` → `all_deps_unstable == false` → `!false` → **the effect is skipped and a possible infinite loop is never reported**. Verified by code trace; a real soundness FN (the cardinal sin) in a *shared* helper, not a rare pattern. Root cause: the ⊤→may fold lives in each caller instead of in one primitive. Fix: fold ⊤/`Versioned` to the "may change" side inside a single shared stability primitive and withdraw `is_unstable()` from the rule surface (route through `may_change` = `!is_stable()`: ⊤/Versioned/PerRender → true); the gate must skip only on a *provably-stable* dep (`is_stable`), never on a merely non-`PerRender` one. Pin with a near-miss fixture (⊤ or `Versioned` dep + unconditional self-write). Part of the in-progress rule-authoring / query-surface ADR.
