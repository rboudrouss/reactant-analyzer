...
| `jsx_props` | Chaque prop de chaque **élément de composant résolu** que le corps de render construit — y compris dans une callback qu'il exécute de façon synchrone, donc `items.map(it => <Row/>)` est énuméré (#125). Les éléments hôtes (`<div/>`) ne produisent aucune ligne, ni un élément construit dans un gestionnaire d'événement. Dans une callback de liste il n'existe aucun env analysé, donc `identity` répond `fresh-every-render` pour une allocation écrite sur place et `unknown` sinon. `name` = l'élément, `prop` = le nom de la prop, `identity` = le verdict d'identité de sa valeur. Pas de `kind`, pas d'arête. |
